...
<div class="max-w-2xl mx-auto space-y-6">
    ...
    <div class="bg-gradient-to-br from-slate-900 via-slate-800 to-brand-900 text-white rounded-3xl p-6 sm:p-8 shadow-xl relative overflow-hidden">
        ...
        <div class="relative z-10 space-y-3">
            <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/10 backdrop-blur-md text-xs font-semibold text-brand-200 border border-white/10">
            ...
            <div class="flex flex-wrap items-center gap-y-2 gap-x-5 pt-3 border-t border-white/10 text-xs text-slate-300">
                ...
                @if($event->instructor)
                    <div class="flex items-center gap-1.5">
                        ...
                        <span>Facilitador: <strong class="text-white">{{ $event->instructor }}</strong></span>
                    </div>
                @endif
    ...
    @if(!$event->allow_registration || $event->status === 'cancelled')
        <div class="bg-white rounded-3xl p-8 border border-slate-200 text-center space-y-4">
            ...
            <h2 class="text-xl font-bold text-slate-900">Registro Cerrado</h2>
            <p class="text-sm text-slate-500 max-w-md mx-auto">El registro de firmas y asistencia para este evento no está disponible en este momento.</p>
        </div>
    @else
